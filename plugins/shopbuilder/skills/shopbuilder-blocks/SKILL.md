...
A placed `newStore` or `faq` block is not a wired one: a fresh `newStore` renders Xsolla demo
data ("Mystic Kit") until its placeholder section is replaced, and both blocks' sections and
...
- Deleting blocks to remove the top nav. The header nav chrome (Store / Daily gifts / Rewards /
  Redeem code) is not in the landing structure or features and persists even after the matching
  blocks are deleted — it cannot be removed via CLI. Surface this limitation to the developer;
