...
Out of scope: catalog & purchase (→ `catalog-design`), Payments (→ `headless-checkout-integration`), webhook handler implementation (→ `webhooks-impl`), Xsolla account/API-key bootstrap (→ `merchant-setup`).
