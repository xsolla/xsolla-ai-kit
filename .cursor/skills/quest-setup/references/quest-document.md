...
| `start_date` | RFC3339 | **only when `active`** | today or later; see below |
...
| `has_personalization` | bool | server-derived | derived from the nodes; do not set it |
| `sample_data` | any | no | accepted, not validated, **not persisted** |

A write that breaks one of the conditional requirements returns a 422.

A `start_date` too far in the past is rejected with a 422 whose message shows
only a date, not a time, so it can look wrong. Use today's date or later.
...
action and that no intended node is orphaned. The API checks references and
cycles, but those checks alone do not prove that the quest does what the
developer wants.
